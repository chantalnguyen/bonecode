% Generates inp file from topology-optimized 2D network for Abaqus Explicit
function generate_inp_from_2Dnet_topopt_failure(node,link,filename,radMult,res)
% takes as input:
% node: struct with node information
% link: struct with link information
% filename: desired name of output inp file
% radMult: factor by which to multiply radius (optional, default=1)
% res: resolution in mm/pixel (optional, default=0.037)
if nargin < 4
    radMult = 1;
end
if nargin < 5
...
fid = fopen(['~/Dropbox/abaqus/' filename '_failure.inp'],'w');
